...
<div class="flex items-center gap-2" class:flex-row={flexRow}>
	...
	{#if $open}
		<div
			class="shadow z-10 flex max-h-[300px] flex-col overflow-y-auto rounded-2 bg-surface-3 p-1 focus:!ring-0"
			use:melt={$menu}
...
<style lang="postcss">
	...
	.options {
		@apply relative cursor-pointer rounded-1 bg-surface-3 py-1 pl-8 pr-4 text-text-2 focus:z-10 focus:text-text-1 data-[highlighted]:bg-primary-3 data-[selected]:bg-primary-10 data-[highlighted]:text-text-1 data-[selected]:text-text-1;
	}
</style>
