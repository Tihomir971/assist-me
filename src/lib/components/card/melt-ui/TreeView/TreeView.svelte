...
{#each treeItems as { id, text, icon, children }}
	...
	<li class={hasChildren ? 'pl-4' : 'pl-9'}>
		<button
			...
			class="rounded-md !bg-transparent shadow-none focus:!bg-magnum-200 focus:!text-orange-600 active:!bg-magnum-200 active:!text-orange-600 flex min-h-[1.5rem] w-full flex-1 items-center p-0 text-left"
			use:melt={$item({
				id: itemId,
				hasChildren
			})}
