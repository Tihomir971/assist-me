<script lang="ts">
	...
	export let error: boolean | null | undefined;
	...
	export let readonly: boolean | null | undefined;
	...
	export let filter = (text: string) => {
		...
		return options.reduce((a: Option[], o) => {
	...
	async function onInputKeyup(event: KeyboardEvent) {
		...
		switch (event.key) {
			...
			case 'Shift':
				break;
			case 'ArrowDown':
				await showList(inputTarget.value);
	...
	function onOptionClick(event: MouseEvent) {
		const inputTarget = event.target as HTMLInputElement;
		if (!inputTarget.matches(`[role="option"]:not([aria-disabled="true"])`)) return;

		selectOption(inputTarget);
		hideList();
	}
	...
	function onListKeyDown(event: KeyboardEvent) {
		...
		switch (event.key) {
			case 'ArrowUp':
				let prevOptionElement = inputTarget.previousElementSibling;
			...
			case 'ArrowDown':
				let nextOptionElement = inputTarget.nextElementSibling;
	...
	function selectOption(optionElement: HTMLInputElement) {
